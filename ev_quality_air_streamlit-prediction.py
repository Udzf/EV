...
def analyze_pollutants(data, feature='% of EV'):
    for target in targets:
        ...
        st.write(f"Linear R²: {r2_linear:.3f}, RMSE: {rmse_linear:.3f}")
        st.write(f"Polynomial R²: {r2_poly:.3f}, RMSE: {rmse_poly:.3f}")

        # Plot actual vs predictions
        plt.figure()
        plt.scatter(X, y, label="Actual", color="blue")
        plt.plot(X, y_pred_linear, color='red', label="Linear Fit")
        plt.plot(X, y_pred_poly, color='green', linestyle='--', label="Polynomial Fit")
        plt.xlabel(feature)
        plt.ylabel(target)
